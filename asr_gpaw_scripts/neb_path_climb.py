...
def main(path_images = 16, forces=0.05):
    ...
    #initial = read('centrosymmetric_structure/structure.json')
    initial = read('switched_structure/structure.json')
    ...
    for i in range(nu_images):
        ...
        if rank in ranks:
            ...
            calc = GPAW(txt=f'neb_climb_{i}.txt',
    ...
    qn = FIRE(neb, logfile='neb_path_climb.log', trajectory='neb_path_climb.traj')
    qn.run(fmax=forces)
