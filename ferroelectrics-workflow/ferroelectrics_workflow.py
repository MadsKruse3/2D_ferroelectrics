...
The workflow is factored into the following components:
  ...
  - A recipe that computes the Polarization along a linearly interpolated path between 
...
def relax_non_centrosymmetric_structure(folder):
    ...
    if not os.path.exists(f'{folder}/results-asr.polarization_path.json'):
        tasks += [task("asr.relax --fmax 0.001 --fixcell --dont-enforce-symmetry --allow-symmetry-breaking",
                       resources="24:2d", 
                       folder=folder)]
    return tasks
...
def polarization_path(folder):
    ...
    if not os.path.exists('centrosymmetric_structure/results-asr.structureinfo.json'):
        return tasks
    ...
    if not pointgroup in polar_pointgroups:
        return tasks
    ...
    if stochiometry < 5:
        c = 24
        if not "results-asr.polarization_path.json" in list(map(str, folder.iterdir())):
...
def polarization_path2(folder):
    ...
    if stochiometry < 5:
        c = 24
        if not "results-asr.polarization_path2.json" in list(map(str, folder.iterdir())):
...
def structureinfo_subfolder(subfolder):
    ...
    if os.path.exists(f'{subfolder}/results-asr.relax.json'):
        if not "results-asr.structureinfo.json" in list(map(str, subfolder.iterdir())):
            tasks += [task("asr.structureinfo", resources="1:10m", folder=subfolder)]
    return tasks
